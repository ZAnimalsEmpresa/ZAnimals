...
public class UnitHealthBaseBehavior : MonoBehaviour
{
    [SerializeField]
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == GetCounterFaction() && _targetUnit == null)
        {
            ...
            _unitContext.SetStrategy(new HealthStrategy(this.gameObject,_targetUnit, 0.5f));
    ...
    private void OnEnemyDeath()
    {
        ...
        foreach (var hitCollider in hitColliders)
        {
            if (hitCollider.gameObject != gameObject && hitCollider.CompareTag(GetCounterFaction()))
            {
                ...
                _unitContext.SetStrategy(new HealthStrategy(gameObject, _targetUnit, 0.5f));
